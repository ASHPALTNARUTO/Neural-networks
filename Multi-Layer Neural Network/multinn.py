...
class MultiNN(object):
    ...
    def add_layer(self, num_nodes, transfer_function="Linear"):
        ...
        if num_of_layer > 0:
            self.weights.append(Multi_NN(self.weights[num_of_layer - 1].numberOfNodes, num_nodes, transfer_function))
        else:
    ...
    def calculate_loss(self, y, y_hat):
        ...
        :return: loss
        """
        entropy_loss = tf.compat.v1.losses.sparse_softmax_cross_entropy(y, y_hat)
        return entropy_loss
    ...
    def predict(self, X):
        ...
        """
        num_of_layer = len(self.weights)
        for layerNumber in range(num_of_layer):
    ...
    def train(self, X_train, y_train, batch_size, num_epochs, alpha=0.8):
        ...
        while epoch < num_epochs:
            ...
            for datapoints, (x, y) in enumerate(dataset):
                ...
                for num_layer in range(num_of_layer):
    ...
    def calculate_percent_error(self, X, y):
        ...
        """
        predicted_value = tf.math.argmax(self.predict(X), axis=1, output_type=tf.dtypes.int32)
        percent_error = 1 - ((tf.math.count_nonzero(tf.math.equal(tf.constant(y) ,predicted_value)).numpy()) / y.shape[0])
        return percent_error
    ...
    def calculate_confusion_matrix(self, X, y):
        ...
        """
        predicted_value = tf.math.argmax(self.predict(X), axis=1, output_type=tf.dtypes.int32)
        confusionTensor = tf.math.confusion_matrix(tf.constant(y), predicted_value, num_classes=10).numpy()
